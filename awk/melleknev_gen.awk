...
/[aáoóuú]$/ { print "[adj]" $1 "/AŇŐĎ/F/U/Î/Q" }
/[öőüű]$/ { print "[adj]" $1 "/CÔŐŢ/H/W/Ě/R" }
/[eéií]$/ { if (mely[$1]==1) {print "[adj]" $1 "/AŇŐĎ/F/U/Î/Q"; next } 
    else {print "[adj]" $1 "/BÓŐĐ/G/V/Ë/R"; next } }
...
/^.*[uúoóaá][bcdfghjklmnpqrstvwxyz]*i[bcdfghjklmnpqrstvwxyz]+$/)) {
    ...
    s2 = fn_s[$1]?"ő":"nň"
    if (ingadozo[$1]==1) { 
        print "[adj]" $1 "/ŇŮ/F/U/Î" s2 a_koto($1,"/KÖ") jaje_e($1,"/Q","/S/s") "/Ó×/G/V/Ë" s a_koto($1,"/L") jaje_e($1,"/R","/T/t") ka("kl") }
    else { print "[adj]" $1 "/ŇŮ/F/U/Î" s a_koto($1,"/KÖ") jaje_e($1,"/Q","/S/s") ka("k") }
    next }

/[iíeéa][-bcdfghjklmnpqrstvwxyz]+$/ {
  s = fn_s[$1]?"ő":"nň"
  s2 = fn_s[$1]?"ö":"mô"
  if (mely[$1]==1) {print "[adj]" $1 "/ŇŮ/F/U/Î" s2 a_koto($1,"/KÖ") jaje_e($1,"/Q","/S/s") ka("k")} 
  else { 
     if (ingadozo[$1]==1) { 
        print "[adj]" $1 "/ŇŮ/F/U/Î" s2 a_koto($1,"/KÖ") jaje_e($1,"/Q","/S/s") "/Ó×/G/V/Ë" s a_koto($1,"/L") jaje_e($1,"/R","/T/t") ka("kl") }
...
/[öőüű][bcdfghjklmnprstvxyz]+$/ { 
  print "[adj]" $1 "/Ô×Ýň/G/V/Ě/N" a_koto($1,"/M") jaje_e($1,"/R","/T/t") ka("l"); 
}
